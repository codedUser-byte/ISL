...
//  WEBSOCKET CONFIG
// ─────────────────────────────────────────────
const _kWsUrlWeb    = 'ws://127.0.0.1:8000/ws';
const _kWsUrlMobile = 'ws://10.0.2.2:8000/ws';
...
class _TranslateScreenState extends State<TranslateScreen>
  ...
  void initState() {
    ...
    WidgetsBinding.instance
        .addPostFrameCallback((_) => _showInstructions());
  }
  ...
  Future<bool> _initCamera() async {
    ...
    try {
      _camCtrl = CameraController(
        ...
        enableAudio: false,
        imageFormatGroup:
            kIsWeb ? null : ImageFormatGroup.jpeg,
      );
  ...
  Future<bool> _connectWebSocket() async {
    try {
      final url = kIsWeb ? _kWsUrlWeb : _kWsUrlMobile;
    ...
    try {
      ...
      if (type == 'prediction') {
        ...
        setState(() {
          ...
          _confidence      = conf;
          _frameCount      = (data['frame'] as int?) ?? _frameCount;
        });
      }
    } catch (e) {
  ...
  Future<void> _stopSession() async {
    if (_state == _SessionState.idle ||
        _state == _SessionState.stopping) return;
  ...
  void _appendToTranscript() {
    if (_predictionLabel == '—' ||
        ...
        _predictionLabel.isEmpty) return;
    ...
    return Scaffold(
      ...
      body: Stack(children: [
        Positioned(
          top: -100, left: -100,
          child: _BlurGlow(
              color: primary.withOpacity(isDark ? 0.14 : 0.08), size: 450),
        ),
        SafeArea(
  ...
  Widget _buildMobileLayout(bool isDark, Color primary) => Column(children: [
  ...
  Widget _currentDetectionSection(bool isDark, Color primary) {
    ...
    return _GlassContainer(
      ...
      primary: primary,
      gradient:
          LinearGradient(colors: [primary.withOpacity(0.08), Colors.transparent]),
      child: Column(children: [
        Text(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ...
          _buildIconAction(Icons.delete_outline_rounded,
              const Color(0xFFEF4444), _textCtrl.clear),
        ]),
      ]),
    );
  }
  ...
  Widget _buildStartStopButton(
      bool isDark, Color primary, AppLocalizations l) {
    final isRunning  = _state == _SessionState.running;
    final isLoading  = _state == _SessionState.connecting ||
                       _state == _SessionState.stopping;
        ...
        if (isLoading)
          SizedBox(
            ...
            child: CircularProgressIndicator(
                strokeWidth: 2, valueColor: AlwaysStoppedAnimation(btnColor)),
          )
        else
          Icon(
            isRunning
  ...
  Widget _buildGradientButton({
    ...
    required VoidCallback onPressed,
      ...
      Container(
        decoration: BoxDecoration(
          gradient:
              LinearGradient(colors: [primary, const Color(0xFF4F46E5)]),
          borderRadius: BorderRadius.circular(14),
          boxShadow: [
            BoxShadow(
    ...
    return BackdropFilter(
      ...
      child: AlertDialog(
        ...
        title: Column(children: [
          ...
          const SizedBox(height: 16),
          Text(l.t('dialog_title'),
              textAlign: TextAlign.center,
              style: TextStyle(
                  fontWeight: FontWeight.w900,
                  fontSize: 18,
                  color:
                      isDark ? Colors.white : const Color(0xFF0A0A1F))),
        ]),
        content: Text(
          l.t('dialog_body'),
          textAlign: TextAlign.center,
